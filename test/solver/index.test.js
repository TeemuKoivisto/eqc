...
function expectSolutionToBe(solution, result, constlength, constindex, key, solutionlength, values) {
  expect(solution.result).to.equal(result);
  expect(solution.constiables.length).to.equal(constlength);
  expect(solution.constiables[constindex].key).to.equal(key);
  expect(solution.constiables[constindex].solutions.length).to.equal(solutionlength);
  for(let i = 0; i < values.length; i++) {
    expect(solution.constiables[constindex].solutions[i]).to.equal(values[i]);
  }
}
...
describe("CalculatorSolver", () => {
  describe("solveEquationUnlogged(latex) should solve", () => {
    describe("(simple, single)", () => {
      ...
      xit("\\left(2+3\\right)+x-\\frac{3}{2}+9=123\\cdot 23+\\frac{2}{2}", () => {
      ...
      xit("\\frac{1}{2}+\\frac{\\frac{2}{9}}{\\frac{3}{12}}=n", () => {
