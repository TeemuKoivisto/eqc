...
var jQuery = window.jQuery,
  Logger = window.Logmon.createOpt('Logmq', false, true),
  undefined,
...
var Point = P(function(_) {
  ...
  _.init = function(parent, leftward, rightward) {
    Logger.append('Tree(Point) init: parent ' + parent + ' leftward ' + leftward + ' rightward ' + rightward);
    this.parent = parent;
    this[L] = leftward;
    this[R] = rightward;
  };
...
var Node = P(function(_) {
  ...
  _.init = function() {
    ...
    Logger.append('Tree(Node) init: VALUE id === ' + this.id);
...
var Fragment = P(function(_) {
  _.init = function(withDir, oppDir, dir) {
    // Logger.start('Fragment init: withDir ' + withDir + ' oppDir ' + oppDir + ' dir ' + dir);
    ...
    if (!withDir) {
        // Logger.end('FROM Fragment init: withDir ' + withDir + ' oppDir ' + oppDir + ' dir ' + dir);
        return;
    }
  ...
  _.adopt = function(parent, leftward, rightward) {
    // Logger.start('Fragment adopt: parent ' + parent + ' leftward ' + leftward + ' rightward ' + rightward);
    ...
    if (!leftEnd) {
        Logger.end('FROM Fragment adopt: parent ' + parent + ' leftward ' + leftward + ' rightward ' + rightward + ' RETURN this ' + this);
        return this;
    }
    ...
    // Logger.end('FROM Fragment adopt: parent ' + parent + ' leftward ' + leftward + ' rightward ' + rightward + ' RETURN self ' + self);
    return self;
  };
...
var Controller = P(function(_) {
  _.init = function(API, root, container) {
    Logger.start('Controller init: API ' + API + ' root ' + root + ' container ' + container);
    this.API = API;
    this.root = root;
    ...
    Logger.end('FROM Controller init: API ' + API + ' root ' + root + ' container ' + container);
    // TODO: stop depending on root.cursor, and rm it
  };

  _.handle = function(name, dir, special) {
    Logger.start('Controller handle: name ' + name + ' dir ' + dir + ' special ' + special);
    var handlers = this.API.__options.handlers;
    if (handlers && handlers[name]) {
//        console.log("handling this shit " + name + " " + dir + " " + handlers);
      if (dir === L || dir === R) { 
        handlers[name](dir, this.API);
      // added special variable for getting the keyevent outside mathquill
      // hacked
      } else if (name === 'captureKeyEvent') {
        handlers[name](special); 
      } else if (name === 'afterCaptureKeyEvent') {
        // also needs this for refreshing the latex after keyevent has been handled
        handlers[name](); 
      // end
      } else handlers[name](this.API);
    }
    Logger.end('FROM Controller handle: name ' + name + ' dir ' + dir + ' special ' + special);
  };
...
var AbstractMathQuill = P(function(_) {
  ...
  _.reflow = function() {
    Logger.append('PublicAPI reflow: RETURN this ' + this);
...
var Parser = P(function(_, super_, Parser) {
  ...
  _.parse = function(stream) {
    Logger.append('Parser.util parse: stream ' + stream);
...
var saneKeyboardEvents = (function() {
  ...
  function stringify(evt) {
    Logger.append('saneKeyboardEvents stringify: evt ' + evt);
    ...
    modifiers.push(key);
//    console.log("mikä on on stringify " + modifiers);
  ...
  return function saneKeyboardEvents(el, handlers) {
    ...
    function select(text) {
      Logger.append('saneKeyboardEvents.util select: text ' + text);
    ...
    function onKeydown(e) {
      Logger.start('saneKeyboardEvents onKeydown: e ' + e);
//        console.log("keydown syvällä... so deep " + e.charCode);
      ...
      handleKey();
      Logger.end('FROM saneKeyboardEvents onKeydown: e ' + e);
    }
...
Controller.open(function(_) {
  _.keystroke = function(key, evt) {
    Logger.start('Controller keystroke: key ' + key + ' evt ' + evt);
    // hacked
    if (this.API.__options.capturingKeyEvents) {
      this.handle('captureKeyEvent', "", {key: key, evt: evt});
    }
    // end
    this.cursor.parent.keystroke(key, evt, this);
    Logger.end('FROM Controller keystroke: key ' + key + ' evt ' + evt);
  };
});

Node.open(function(_) {
  _.keystroke = function(key, e, ctrlr) {
    Logger.start('Keystroke keystroke: key ' + key + ' e ' + e + ' ctrlr ' + ctrlr);
    ...
    default:
      Logger.end('FROM Keystroke keystroke: key ' + key + ' e ' + e + ' ctrlr ' + ctrlr + ' RETURN void');
      return;
    }
    e.preventDefault();
    ctrlr.scrollHoriz();
...
Controller.open(function(_, super_) {
  ...
  _.writeLatex = function(latex) {
    Logger.start('Latex writeLatex: latex ' + latex);
    ...
    Logger.end('FROM Latex writeLatex: latex ' + latex + ' RETURN this ' + this);
    return this;
  };
  _.renderLatexMath = function(latex) {
    Logger.start('Latex renderLatexMath: latex ' + latex);
    ...
    cursor.insAtRightEnd(root);
    Logger.end('FROM Latex renderLatexMath: latex ' + latex);
  };
...
Controller.open(function(_) {
  ...
  _.typedText = function(ch) {
    Logger.start('Textarea typedText: ch ' + ch);
    // removed that unnecessary handle of enters
    if (ch === '\n') {
        Logger.end('FROM Textarea typedText: RETURN void CONDITION ch === \'\\n\'');
        return;
    }
    var cursor = this.notify().cursor;
    cursor.parent.write(cursor, ch);
    this.scrollHoriz();
    this.handle('afterCaptureKeyEvent');
    Logger.end('FROM Textarea typedText: ch ' + ch);
  };
...
var MathElement = P(Node, function(_, super_) {
  _.finalizeInsert = function(options, cursor) { // `cursor` param is only for
    Logger.start('Math(MathElement) finalizeInsert: options ' + options + ' cursor ' + cursor);
    ...
    self.bubble('reflow');
    Logger.end('FROM Math(MathElement) finalizeInsert: options ' + options + ' cursor ' + cursor);
  };
});
...
var MathCommand = P(MathElement, function(_, super_) {
  _.init = function(ctrlSeq, htmlTemplate, textTemplate) {
    Logger.start('Math(MathCommand) init: ctrlSeq ' + ctrlSeq + ' htmlTemplate ' + htmlTemplate + ' textTemplate ' + textTemplate);
    var cmd = this;
    ...
    Logger.end('FROM Math(MathCommand) init: ctrlSeq ' + ctrlSeq + ' htmlTemplate ' + htmlTemplate + ' textTemplate ' + textTemplate);
  };
...
var MathBlock = P(MathElement, function(_, super_) {
  ...
  _.keystroke = function(key, e, ctrlr) {
    Logger.start('MathBlock keystroke: key ' + key + ' e ' + e + ' ctrlr ' + ctrlr);
    if (ctrlr.API.__options.spaceBehavesLikeTab
        && (key === 'Spacebar' || key === 'Shift-Spacebar')) {
      e.preventDefault();
      ctrlr.escapeDir(key === 'Shift-Spacebar' ? L : R, key, e);
      Logger.end('FROM MathBlock keystroke: key ' + key + ' e ' + e + ' ctrlr ' + ctrlr + ' RETURN void CONDITION SpaceBehavesLikeTab && key === \'space\'');
      return;
    }
    Logger.end('FROM MathBlock keystroke: key ' + key + ' e ' + e + ' ctrlr ' + ctrlr + ' RETURN super_.keystroke.apply(this, argument)');
...
Logger.append('Commands transformPropName ' + transformPropName);
