Calculator.service('CalculatorSolverLogic', function (CalculatorMain, CalculatorUtil, CalculatorBasic, MathSymbol, MathTerm, MathOperation, MathBracketed, LatexParser, Logger) {
    ...
    this.solve = function(latex) {
        ...
        if (parser.status.length !== 0) {
            return this.composeSolution(eq, 'unable', 'Parser error', { message: parser.status });
        }
    ...
    this.solveTwoTermEquation = function(equation) {
        ...
        if (exvalue === 9999) {
            solutionCore = { result: 'unable', code: 'Non term exp' };
    ...
    this.solveThreeTermEquation = function(equation) {
        ...
        if (equation.leftside[0].getPossibleExponentValue() !==2) {
            solutionCore = { result: 'unable', code: 'Large exp' };
        ...
        if (equation.leftside[0].getPossibleExponentValue()===2 && equation.leftside[1].getPossibleExponentValue()===1 && equation.leftside[2].getPossibleExponentValue()===1) {
            ...
            solutionCore = { result: 'unable', message: 'Quadratic fail' };
        }
        Logcal.end("FROM solveThreeTermEquation: equation " + equation + " RETURN solutionCore " + solutionCore);
        return solutionCore;
    };
    ...
    this.solveTermEquation = function(equation) {
        ...
        if (variables.length>1) {
            // too many variables
            return { result: 'unable', code: 'Many vars'};
        }
        ...
        if (equation.leftside.length===1) {
            ...
            if (variables.length === 1) {
                ...
                solutionCore.variables = [ { key: equation.leftside[0].variable, solutions: ['0'] } ];
                ...
                if (equation.leftside[0].getValue() === 0) {
                    ...
                    solutionCore.variables = [ { key: 'constant', solutions: ['0'] } ];
                } else {
                    solutionCore.result = 'false',
                    solutionCore.code = 'Consts not canceled';
                    solutionCore.variables = [ { key: 'constant', solutions: ['' + equation.leftside[0].getValue() ] } ];
                }
            }
            return solutionCore;
            ...
            return { result: 'unable', code: 'Many exps' };
        }
    };
});
